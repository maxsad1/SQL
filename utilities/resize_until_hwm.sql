 ...
 * 1.1		  Anjul Sahu		06-Oct-2014		   Skipping datafile which are not being changed
 *****************************************************************************************************/
...
loop
    ...
    v_skip_datafile := FALSE; 
    
    IF (c_report.currsize < v_new_size) THEN
        v_new_size := c_report.currsize; 
        v_skip_datafile := TRUE;
    END IF;
       
    IF (v_execute_cmd = TRUE AND v_skip_datafile = FALSE ) THEN
    BEGIN
